    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {!user ? (<button onClick={() => setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary/80 transition text-white rounded-full">
                    Login
                </button>)
                    :
            ...
            {open && (
                <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                    ...
                    <NavLink onClick={() => setOpen(false)} to='/allproducts' className="block">All Products</NavLink>
                    {user &&
                        <NavLink onClick={() => setOpen(false)} to='/myorders' className="block">My Orders</NavLink>
                    }
                    <NavLink onClick={() => { setOpen(false); setShowUserLogin(true) }} to='/' className="block">Order</NavLink>
                    {!user ? (
